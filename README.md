...
- 支持删除切图框，并通过数值输入或步进按钮微调选区位置和尺寸
- 支持从画布点击吸取背景颜色
- 支持画布滚轮缩放、按钮缩放、适应窗口和右键/中键拖拽平移
- 支持调整切图结果缩略图大小，并在横向滚动和自动换行布局间切换
...
- “左上角”适合左上角确定是背景色的图片。
- “手动 RGB”适合需要指定精确背景色的图片，可以点击“取左上角”快速填入颜色，也可以点击“从画布吸色”后直接在预览图上取色。
- “容差”越大，越多接近背景色的像素会被移除。
- “羽化”越大，边缘越柔和，但也更容易产生半透明区域。
- “去白边”会反推抗锯齿边缘的 Alpha，并减轻残留背景色污染。白底黑线稿建议从 `10~40` 之间调节。
- “边缘收缩”会向内收缩 Alpha 边缘。仍有一圈明显白边时，可以先设为 `1`，细节太细时再调回 `0`。
...
自动切图后，可以点击画布上的 Bounding Box，或点击底部缩略图选中切图框。选中后可在“选区编辑”面板中：
